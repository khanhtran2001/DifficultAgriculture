...
RESULTS_DIR = Path(f"results/{Path(__file__).stem}_minneapple_val_score")
CONFIG_DIR = Path("/home/khanh/Projects/DifficultyAgri/configs/experiments/minneapple_yolo.yaml")
DATASET_PROPERTIES_PATH = Path(
    "/home/khanh/Projects/DifficultyAgri/.cache_result/no_trad_aug/minneapple_v2/dataset_properties.json"
)
MODEL_WEIGHT_PATH = Path(
    "/home/khanh/Projects/DifficultyAgri/.cache_result/no_trad_aug/minneapple_v2/baseline/seed_123/best.pt"
)
...
def run_experiment(config_path: str, dataset_properties_path: str, model_weight_path: str):
    ...
    max_det = compute_max_det_from_train_labels(
        train_labels_dir=initial_dataset_properties.val_labels_dir,
        percentile=0.99,
        multiplier=3,
    )
    print(f"Auto max_det from p99 object count x3: {max_det}")
    image_dir = initial_dataset_properties.val_images_dir
    ...
    # Then we can score the validation dataset using the predictions and save the results
    """
    ...
    score_results = scoring.score(
        ...
        images_dir=initial_dataset_properties.val_images_dir,
        labels_dir=initial_dataset_properties.val_labels_dir,
    )
