...
5. Run: python scripts/download_dota_v1.py --skip-download
...
Run this command to attempt automatic download:
  ...
  python scripts/download_dota_v1.py
...
4. Run: python scripts/setup_dota.py --verify
...
def main():
	...
	if args.auto:
		...
		print(f"\nTrying to run: python {PROJECT_ROOT}/scripts/download_dota_v1.py")
		try:
			subprocess.run(
				[sys.executable, str(PROJECT_ROOT / "scripts" / "download_dota_v1.py")],
				check=True
			)
